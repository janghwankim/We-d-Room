			   ...
			   //sql="select * from (select * from Notice order by free_no desc) where rownum >= ? and rownum <= ?";
				...
				if(search.equals("all")) {
					sql="SELECT a.*  \n"
							...
							searchtext+"%' or content like '%"+searchtext+"%' order by notice_no desc) f)a,\n"
							        + "(select @rnum:=0) AS R\n"
							        + " where rnum > ? and rnum <= ?";
				}else {
				   sql="SELECT a.*  \n"
				           + "FROM (SELECT @rnum := @rnum+1 AS rnum, f.*  \n"
				           + "FROM  (SELECT * \n"
				           + "FROM Notice where  title like '%"+search+"%' or content like '%"+
						   searchtext+"%' order by notice_no desc) f)a,\n"
						           + "(select @rnum:=0) AS R\n"
						           + " where rnum > ? and rnum <= ?";
				}
			}
